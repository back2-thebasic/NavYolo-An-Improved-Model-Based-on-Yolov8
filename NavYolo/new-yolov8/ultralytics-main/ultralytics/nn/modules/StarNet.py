...
class StarNet(nn.Module):
    def __init__(self, depth=0.25, width=0.5, base_dim=32, depths=[3, 3, 12, 5], mlp_ratio=4, drop_path_rate=0.0,
...
class StarNet_s(nn.Module):
    def __init__(self, number):
        super(StarNet_s, self).__init__()
        self.model = None
        if number == 1:
            self.model = starnet_s1().stem  # 提取输出特征图大小为 320 * 320 的部分
        elif number == 2:
            ...
            self.model = nn.Sequential(
                *starnet_s1().stages[1:2],
            )
        elif number == 4:
            self.model = nn.Sequential(
                *starnet_s1().stages[2:3],
            )
        else:
            self.model = nn.Sequential(
                *starnet_s1().stages[3:],
...
if __name__ == "__main__":
    base_model = starnet_s1()


    summary(base_model, (1,3, 640, 640))