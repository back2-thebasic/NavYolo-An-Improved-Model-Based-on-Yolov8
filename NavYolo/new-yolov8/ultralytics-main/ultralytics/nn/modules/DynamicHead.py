...
from ultralytics.nn.modules import Proto, Detect,Conv, DFL
...
class DetectDynamicHead(Detect):
    def __init__(self, nc=80, ch=()):
        ...
        for i in range(self.nl):
            channel = ch[i]
            DynamicHead_tower.append(DyConv(channel,channel,conv_func=Conv3x3Norm))
        self.add_module('DynamicHead_tower', nn.Sequential(*DynamicHead_tower))

    def forward(self, x):
        tensor_dict = {i: tensor for i, tensor in enumerate(x)}
        x = self.DynamicHead_tower(tensor_dict)
        x = list(x.values())
...
class SegmentDynamicHead(DetectDynamicHead):
    def __init__(self, nc=80, nm=32, npr=256, ch=()):
        ...
        self.detect = DetectDynamicHead.forward
