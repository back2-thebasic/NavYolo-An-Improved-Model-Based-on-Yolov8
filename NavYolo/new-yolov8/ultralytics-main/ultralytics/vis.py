...
def print_precision(csv_dict, file_names):
    # 提取指定列
    column_name = '   metrics/precision(B)'
    # 曲线平滑
    def moving_average(interval, windowsize):
        p = k // 2
        # 首尾元素重复 p 次
...
def print_pg0(csv_dict, file_names):
    # 提取指定列
    column_name = '                 lr/pg0'
    # 曲线平滑
    def moving_average(interval, windowsize):
...
if __name__ == '__main__':
    csv_files = get_csv_files("E:/su/csv_data/compare")
    csv_dict,  file_names = read_csv_files_to_dict(csv_files)
    #print_mAP50_95(csv_dict, file_names)
    print_mAP50(csv_dict, file_names)
    #print_precision(csv_dict, file_names)
    #print_pg0(csv_dict, file_names)
